...
public class PersonTest {
	...
	public void insertAndRetrieve() {
		...
		((JBEntityManager)em).setFlushListener(c);
		...
		final List<Person> list = em.createNativeQuery(
				"select ?s where { ?s a <http://test.jpa/Person> }")
				.getResultList();
		...
		for (Person current : list) {
			...
			current.setFirstName("sam");
			
		}
		em.flush();
		assertEquals(2, c.count);
		((JBEntityManager)em).getModel().write(System.out, "N3");
